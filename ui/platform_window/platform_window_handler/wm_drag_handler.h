...
class WmDragHandler {
 public:
  ...
  // |operation| is the suggested operation to the destination and the
  // destination sets the final operation when the drop action is performed.
  virtual void StartDrag(const OSExchangeData& data,
                         const int operation,
                         gfx::NativeCursor cursor) = 0;
