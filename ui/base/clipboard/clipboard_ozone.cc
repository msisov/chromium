...
class ClipboardOzone::AsyncClipboardOzone {
 public:
  ...
  std::vector<uint8_t> ReadClipboardDataAndWait(ClipboardType type,
    ...
    // TODO(tonikitoo): add primary selection support.
 ...
 private:
  ...
  // Cached clipboard data, which is pending to be written. Must be cleared on every new write to a clipboard.
...
uint64_t ClipboardOzone::GetSequenceNumber(ClipboardType type) const {
  // TODO(msisov): add sequence number support.
  NOTIMPLEMENTED();
  return 0;
}
