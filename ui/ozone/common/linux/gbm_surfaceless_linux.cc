...
GbmSurfacelessLinux::GbmSurfacelessLinux(gfx::AcceleratedWidget widget)
...
void GbmSurfacelessLinux::QueueOverlayPlane(const OverlayPlane& plane) {
  is_on_external_drm_device_ = plane.buffer->RequiresGlFinish();
  planes_.push_back(plane);
}

gfx::SwapResult GbmSurfacelessLinux::SwapBuffers(
    const PresentationCallback& callback) {
  NOTREACHED();
  return gfx::SwapResult::SWAP_FAILED;
}

bool GbmSurfacelessLinux::ScheduleOverlayPlane(
    int z_order,
...
bool GbmSurfacelessLinux::IsOffscreen() {
  return false;
}

bool GbmSurfacelessLinux::SupportsPresentationCallback() {
  return true;
}

bool GbmSurfacelessLinux::SupportsAsyncSwap() {
  return true;
}

bool GbmSurfacelessLinux::SupportsPostSubBuffer() {
  // false for wayland
  return true;
}

gfx::SwapResult GbmSurfacelessLinux::PostSubBuffer(
    int x,
    int y,
    int width,
    int height,
...
void GbmSurfacelessLinux::SwapBuffersAsync(
    const SwapCompletionCallback& completion_callback,
    const PresentationCallback& presentation_callback) {
  TRACE_EVENT0("drm", "GbmSurfacelessLinux::SwapBuffersAsync");
  ...
  // GbmSurfacelessLinux::SubmitFrame.
  ...
  base::OnceClosure fence_retired_callback = base::BindOnce(
      &GbmSurfacelessLinux::FenceRetired, weak_factory_.GetWeakPtr(), frame);
...
void GbmSurfacelessLinux::PostSubBufferAsync(
    int x,
    int y,
    int width,
    int height,
...
EGLConfig GbmSurfacelessLinux::GetConfig() {
  if (!config_) {
...
void GbmSurfacelessLinux::SetRelyOnImplicitSync() {
  use_egl_fence_sync_ = false;
}

void GbmSurfacelessLinux::SetUsePlaneGpuFences() {
  use_egl_fence_sync_ = false;
}

GbmSurfacelessLinux::~GbmSurfacelessLinux() {}

GbmSurfacelessLinux::PendingFrame::PendingFrame() {}

GbmSurfacelessLinux::PendingFrame::~PendingFrame() {}

bool GbmSurfacelessLinux::PendingFrame::ScheduleOverlayPlanes(
...
void GbmSurfacelessLinux::PendingFrame::Flush() {
  for (const auto& overlay : overlays)
    overlay.Flush();
}

void GbmSurfacelessLinux::SubmitFrame() {
  ...
  if (unsubmitted_frames_.front()->ready && !swap_buffers_pending_) {
    ...
    auto callback = base::BindOnce(&GbmSurfacelessLinux::SwapCompleted,
    ...
    SchedulePageFlip(planes_, std::move(callback));
    planes_.clear();
  }
}

EGLSyncKHR GbmSurfacelessLinux::InsertFence(bool implicit) {
...
void GbmSurfacelessLinux::FenceRetired(PendingFrame* frame) {
  frame->ready = true;
  SubmitFrame();
}

void GbmSurfacelessLinux::SwapCompleted(
