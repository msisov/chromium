...
void WaylandConnection::ScheduleBufferSwap(const gfx::AcceleratedWidget& widget,
  ...
  ui_task_runner_->PostTask(FROM_HERE,
                            base::BindOnce(&WaylandConnection::ScheduleFlush,
...
//                                             uint32_t buffer_id) {
base::File file, uint32_t width, uint32_t height, const std::vector<uint32_t>& strides, const std::vector<uint32_t>& offsets, uint32_t format, const std::vector<uint64_t>& modifiers, uint32_t planes_count, uint32_t buffer_id) {
  // The |file| will be destroyed once it goes out the function scope. Thus,
  // post a task to avoid blocking mojo.
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE,
      base::BindOnce(&WaylandConnection::CreateZwpLinuxDmabufInternal,
                     base::Unretained(this), base::Passed(std::move(file)),
                     width, height, strides, offsets, format, modifiers, planes_count,
                     buffer_id));
}

void WaylandConnection::CreateZwpLinuxDmabufInternal(
// base::File file,
//                                                     uint32_t width,
//                                                     uint32_t height,
//                                                     uint32_t strides[],
...
base::File file, uint32_t width, uint32_t height, const std::vector<uint32_t>& strides, const std::vector<uint32_t>& offsets, uint32_t format, const std::vector<uint64_t>& modifiers, uint32_t planes_count, uint32_t buffer_id) {
  ...
  for (int i = 0; i < planes_count; i++) {
    LOG(ERROR) << "Count " << i;
    zwp_linux_buffer_params_v1_add(params, fd, i /* plane id */, offsets[i], strides[i],
...
void WaylandConnection::CreateSucceeded(
  ...
  auto pending_buffers_it = connection->pending_buffer_map_.find(handle);
  if (pending_buffers_it != connection->pending_buffer_map_.end()) {
    ...
    connection->ScheduleBufferSwap(widget, handle);
  }
}
