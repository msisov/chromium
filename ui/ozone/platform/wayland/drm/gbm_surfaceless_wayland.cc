...
void GbmSurfacelessWayland::SchedulePageFlip(
  ...
  // TODO(msisov): add a presentation feedback if supported by Wayland.
  time_of_last_swap_ = base::TimeTicks::Now();
  // Check comment in ::SupportsPresentationCallback.
  std::move(callback).Run(
      gfx::SwapResult::SWAP_ACK,
      gfx::PresentationFeedback(time_of_last_swap_, base::TimeDelta(),
