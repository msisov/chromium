...
GbmSurfaceless::GbmSurfaceless(SurfaceFactoryOzone* surface_factory,
...
void GbmSurfaceless::SubmitFrame() {
  ...
  if (unsubmitted_frames_.front()->ready && !swap_buffers_pending_) {
    ...
    CHECK(!planes_.empty());
    uint32_t handle = planes_.back().buffer->GetHandle();
		planes_.clear();
    surface_factory_->SchedulePageFlip(handle);      
...
void GbmSurfaceless::SwapCompleted(
  ...
  if (result == gfx::SwapResult::SWAP_FAILED) {
    CHECK(false) << "SwapCompleted::SWAP FAILED";
