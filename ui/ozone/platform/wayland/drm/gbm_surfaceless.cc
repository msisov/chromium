...
GbmSurfaceless::GbmSurfaceless(WaylandSurfaceFactory* surface_factory,
...
bool GbmSurfaceless::SupportsPresentationCallback() {
  // TODO(msisov): implement real presentation feedback. At the moment,
  // it's a fake callback about successful buffer swap.
  return true;
}
...
void GbmSurfaceless::SubmitFrame() {
  ...
  if (unsubmitted_frames_.front()->ready && !swap_buffers_pending_) {
    ...
    uint32_t buffer_id = planes_.back().buffer->GetHandle();
