...
GbmBufferWayland::GbmBufferWayland(
  ...
  connection_->CreateZwpLinuxDmabuf(
      std::move(file), GetWidth(), GetHeight(), strides, offsets, get_format(),
      modifiers, plane_count, GetHandle() /* use as buffer id */);
}
...
int GbmBufferWayland::GetBoFd() const {
  return gbm_bo_get_fd(get_bo());
}

uint32_t GbmBufferWayland::GetWidth() const {
  return gbm_bo_get_width(get_bo());
}

uint32_t GbmBufferWayland::GetHeight() const {
  return gbm_bo_get_height(get_bo());
}
