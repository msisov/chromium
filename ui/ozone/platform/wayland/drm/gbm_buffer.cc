...
GbmBuffer::GbmBuffer(const scoped_refptr<GbmDeviceBase>& gbm,
                     gbm_bo* bo,
                     uint32_t format,
                     uint32_t flags,
                     uint64_t modifier,
...
int GbmBuffer::GetFd(size_t index) const {
  // TODO(msisov): seems like linux doesn't provide plane fds,
  // but rather one fd per bo. Fix this.
...
int GbmBuffer::GetOffset(size_t index) const {
  return gbm_bo_get_offset(bo_, index);
  DCHECK_LT(index, planes_.size());
...
scoped_refptr<GbmBuffer> GbmBuffer::CreateBufferForBO(
  ...
  planes.emplace_back(gbm_bo_get_stride_for_plane(bo, 0),
                      gbm_bo_get_offset(bo, 0),
                      gbm_bo_get_height(bo) * gbm_bo_get_stride_for_plane(bo, 0) /* find out if this is correct */,
                      modifier);
...
//               gbm->device_path().value(), "size", size.ToString());
  ...
  gbm_bo* bo =
      gbm_bo_create(gbm->device(), size.width(), size.height(), fourcc_format, flags);
  if (!bo)
    return nullptr;
  return CreateBufferForBO(gbm, bo, fourcc_format, size, flags);
}

GbmPixmap::GbmPixmap(SurfaceFactoryOzone* surface_manager,
...
bool GbmPixmap::ScheduleOverlayPlane(gfx::AcceleratedWidget widget,
  ...
  GbmSurfaceless* surfaceless = static_cast<GbmSurfaceless*>(surface_manager_->GetSurface(widget));
  CHECK(surfaceless);
