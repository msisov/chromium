 ...
 public:
  ...
  scoped_refptr<gl::GLSurface> CreateSurfacelessViewGLSurface(
    ...
    return gl::InitializeGLSurface(
        new GbmSurfaceless(surface_factory_, window));
  }
...
void WaylandSurfaceFactory::RegisterSurface(gfx::AcceleratedWidget widget,
  ...
  widget_to_surface_map_.insert(
      std::make_pair(widget, static_cast<GbmSurfaceless*>(surface)));
}
...
scoped_refptr<gfx::NativePixmap> WaylandSurfaceFactory::CreateNativePixmap(
  ...
  scoped_refptr<GbmBuffer> buffer =
      GbmBuffer::CreateBuffer(connection_, size, format, usage);
  if (!buffer)
    return nullptr;
